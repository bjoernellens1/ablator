...
def test_dispatch_lock_serializes_two_processes(tmp_path, monkeypatch):
    ...
    holder for the SAME machine fail (non-blocking -- see its docstring for
    why not LOCK_EX) while the first holds it, succeed once released, and
    must never contend with a different machine's lock.
    """
    ...
    try:
        assert runner._acquire_dispatch_lock("r9700") is None, (
            "a second process for the SAME machine must NOT acquire the "
            "dispatch lock while the first still holds it -- this is "
            "exactly the gap that let two GPU jobs launch concurrently "
            "on r9700"
        )
...
def test_run_loop_holds_dispatch_lock_only_across_one_tick(tmp_path, monkeypatch):
    ...
    # Lock must be free again: acquiring it fresh must succeed immediately.
    fd = runner._acquire_dispatch_lock(cfgmod.machine_name(cfg))
    assert fd is not None
    os.close(fd)


def test_run_loop_once_returns_promptly_when_lock_held_elsewhere(tmp_path, monkeypatch):
    """A `--once` dispatcher racing an already-running persistent daemon for
    the same machine must return immediately reporting nothing to dispatch
    (like baremetal_busy=True), never block waiting for the other process's
    job to finish -- that would turn a one-shot idle-triggered launcher into
    an hours-long hang."""
    cfg = make_cfg(tmp_path)
    monkeypatch.setattr(runner, "_dispatch_lock_path",
                        lambda machine: str(tmp_path / f"dispatch_{machine}.lock"))
    held_fd = runner._acquire_dispatch_lock(cfgmod.machine_name(cfg))
    ...
    try:
        ...
        assert elapsed < 5.0, (
            f"run_loop(once=True) took {elapsed:.1f}s while another process "
            "held the dispatch lock -- it must return promptly, not block"
        )
    finally:
        os.close(held_fd)
